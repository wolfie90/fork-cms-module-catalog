...
class Model implements FrontendTagsInterface
{
	...
	{
		$items = (array) FrontendModel::getContainer()->get('database')->getRecords(
			'SELECT o.*, c.title, c.price, m.url
	...
	public static function getAll($limit = 10, $offset = 0)
	{
		$items = (array) FrontendModel::getContainer()->get('database')->getRecords(
			'SELECT i.*, m.url
			 ...
			 INNER JOIN meta AS m ON i.meta_id = m.id
			 WHERE i.language = ?
			 ORDER BY i.created_on ASC, i.id DESC LIMIT ?, ?',
	...
	*/
	...
	{
		$items = (array) FrontendModel::getContainer()->get('database')->getRecords(
				'SELECT c.id AS id, c.title, m.url, COUNT(p.id) AS total, m.data AS meta_data, parent_id
	...
	*/
	...
	{		
		...
		{
			$items = (array) FrontendModel::getContainer()->get('database')->getRecords(
				'SELECT c.id, c.title, m.url, COUNT(p.id) AS total, m.data AS meta_data
				 FROM catalog_categories AS c
				 INNER JOIN meta AS m ON c.meta_id = m.id
				 LEFT OUTER JOIN catalog_products AS p ON p.category_id = c.id
		...
		{
			$items = (array) FrontendModel::getContainer()->get('database')->getRecords(
				'SELECT c.id, c.title, m.url, COUNT(p.id) AS total, m.data AS meta_data, parent_id
				 FROM catalog_categories AS c
				 INNER JOIN meta AS m ON c.meta_id = m.id
				 LEFT OUTER JOIN catalog_products AS p ON p.category_id = c.id
				 WHERE c.language = ?
	...
	public static function existsOrderValue($productId, $orderId)
	{
		return (bool) BackendModel::getContainer()->get('database')->getVar(
			'SELECT 1
			 ...
			 LIMIT 1',
			 array((int) $productId, (int) $orderId));
	}	
	/**
	...
	public static function insertComment(array $comment)
	{
		...
		if($comment['status'] == 'published')
		{
			...
			$numComments = (int) FrontendModel::getContainer()->get('database')->getVar(
				...
				array('published', $comment['product_id'], FRONTEND_LANGUAGE)
			);
	...
	*/
	...
	{
		$items = (array) FrontendModel::getContainer()->get('database')->getRecords(
			'SELECT i.id, i.title, i.summary, i.text, m.url
			 FROM catalog_products AS i
			 INNER JOIN meta AS m ON i.meta_id = m.id
