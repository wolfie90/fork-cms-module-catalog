...
class BackendCatalogModel
{
	...
	const QRY_DATAGRID_BROWSE_ORDERS =
		'SELECT
	...
	const QRY_DATAGRID_BROWSE_PRODUCTS_ORDER =
		'SELECT c.title, c.price, o.*, m.url
	...
	const QRY_DATAGRID_BROWSE_SPECIFICATIONS =
	...
	const QRY_DATAGRID_BROWSE_IMAGES =
		'SELECT i.id, i.product_id, i.filename, i.title, i.sequence
	...
	const QRY_DATAGRID_BROWSE_VIDEOS =
		'SELECT i.id, i.product_id, i.embedded_url, i.title, i.sequence
	...
	public static function existsSpecificationValue($productId, $specificationId)
	{
		return (bool) BackendModel::getContainer()->get('database')->getVar(
			'SELECT 1
			 ...
			 LIMIT 1',
			array((int) $productId, (int) $specificationId));
	}
	...
	{
		return (array) BackendModel::getContainer()->get('database')->getRecord(
			'SELECT i.*, UNIX_TIMESTAMP(i.date) AS ordered_on,
	...
	public static function getAllProductsGroupedByCategories()
	{
		...
		$allProducts = (array) $db->getRecords(
	...
	public static function getRelatedProducts($id)
	{
		...
		$relatedProducts = (array) $db->getPairs(
			'SELECT r.related_product_id AS keyId, r.related_product_id AS valueId
	...
	public static function getSpecificationValue($specificationId, $productId)
	{
		...
		$value = (array) $db->getRecord(
			'SELECT i.value
			 FROM catalog_specifications_values i
			 WHERE i.specification_id = ? AND i.product_id = ?',
			 array((int) $specificationId, (int) $productId));
	...
	public static function categoryHasChildren($id)
	{
		...
		$item = (array) BackendModel::getContainer()->get('database')->getRecord(
			'SELECT c1.id as childID, c1.title ChildName, c2.title as ParentName
			 FROM catalog_categories AS c1
			 LEFT OUTER JOIN catalog_categories AS c2 ON c1.id = c2.parent_id
			 WHERE c1.language = ? AND c1.parent_id = ?',
	...
	public static function reCalculateCommentCount(array $ids)
	{
		...
		$commentCounts = (array) $db->getPairs(
			'SELECT i.product_id, COUNT(i.id) AS comment_count
			 FROM catalog_comments AS i
			 INNER JOIN catalog_products AS p ON i.product_id = p.id AND i.language = p.language
	...
	public static function updateCommentStatuses($ids, $status)
	{
		...
		// get the items and their languages
		$items = (array) BackendModel::getContainer()->get('database')->getPairs(
			'SELECT i.product_id, i.language
			 FROM catalog_comments AS i
			 WHERE i.id IN (' . implode(', ', $idPlaceHolders) . ')',
			$ids, 'product_id'
		);
